...
# Before executing the script, initialize Git repository & clone the bod project from github
# git init
# git clone git@github.com:geoadmin/db.git 
# cd bod

# Now we are inside geoadmin/bod and here the script is executable 
# If permission is denied when executing the bash script, do: chmod u+x <name>
# At the <artefactsBranch> all the changes are commited between the different phases (production - integration phases)
# for different dates (a history of changes between the different phases is being kept)
# For the first time executing the script, the <artefactsBranch> should be created, so do: git branch atrefactsBranch
git checkout prod_vs_int_review

# If a repo does not exist, it is created
...
# if the number of lines is greater than 4: the comparison of json files is possible
if [ $NUMOFLINES -gt 4 ]; then
...
git push origin artefactsBranch
